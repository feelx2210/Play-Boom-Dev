...
function calculateWallScore(gx, gy, range) {
    ...
    DIRS.forEach(d => {
        for (let i = 1; i <= range; i++) {
            ...
            // Regel: Skull-Items wegsprengen - sehr hohe Priorität
 ...
 * Schädliche Items (ITEMS.SKULL) werden ignoriert.
 */
function findGoodItemMove(gx, gy, dangerMap) {
    ...
    while (queue.length > 0) {
        ...
        if (current.dist > 15) continue; // Suche abbrechen wenn zu weit
        ...
        for (let d of DIRS) {
            ...
            if (nx >= 0 && nx < GRID_W && ny >= 0 && ny < GRID_H && !visited.has(key)) {
                // Skulls blockieren den Weg (außer man will sterben)
 ...
 * Priorisiert das "Einsperren" (Bombe legen wenn benachbart).
 */
function findPlayerMove(gx, gy, dangerMap) {
    ...
    // KILLER-LOGIK: Wenn direkt daneben (Distanz 1), Bombe legen!
    // Das sperrt den Spieler in Sackgassen ein.
    if (distToPlayer <= 1) {
        ...
        for (let d of DIRS) {
            ...
            if (nx >= 0 && nx < GRID_W && ny >= 0 && ny < GRID_H && !visited.has(key)) {
                if (!isSolid(nx, ny) && !dangerMap[ny][nx] && state.items[ny][nx] !== ITEMS.SKULL) {
                    ...
                    // Wenn wir ein Feld erreichen, das benachbart zum Ziel ist, ist das gut genug für einen Angriff
                    if (isTarget) {
...
export function updateBotLogic(bot) {
    ...
    if (amInDanger) {
        ...
        if (isHardMode) {
            ...
            // Wenn Spieler nah ist (<= 4 Felder), ignoriere Items und töte ihn! ("Killer"-Modus)
            // Wenn Spieler weit weg ist, sammle erst Items ("Vorbereitung").
            const AGGRO_RANGE = 4;
            let bestMove = null;

            if (targetPlayer && distToPlayer <= AGGRO_RANGE) {
                // Aggressiv: Erst Angriff prüfen
            ...
            // Auswertung des bestMove (Angriff/Jagd)
            if (!targetDir.x && !targetDir.y && bestMove) {
                if (bestMove.isBombSpot || bestMove.shouldPlant) {
                    // Wir stehen gut (z.B. neben dem Spieler in der Sackgasse) -> BOMBEN!
                    if (bot.activeBombs < bot.maxBombs) {
                        // Check: Kann ich danach flüchten?
                        if (canEscapeAfterPlanting(gx, gy, dangerMap)) {
                            bot.plantBomb();
                            targetDir = findSafeMove(gx, gy, getDangerMap());
                        } else {
                            // Wenn ich nicht flüchten kann, warte ich oder laufe weg (nicht suiziden)
                            // Außer: Ich blockiere den Spieler schon? Nein, Sicherheit geht vor.
                            targetDir = {x:0, y:0};
                        }
                    }
                } else {
            ...
            // Strategische Bomben (Farming), falls wir sonst nichts tun (kein Angriff, kein Item)
            if (!targetDir.x && !targetDir.y && bot.activeBombs < bot.maxBombs) {
                const wallScore = calculateWallScore(gx, gy, bot.range);
                // Nur legen, wenn es sich lohnt (Wände oder Skull)
