...
function getCachedSprite(charDef, d, isCursed) {
    ...
    // Helper für detaillierten Stil (wie im Original)
    ...
    // --- NEUE CHARAKTERE (High Detail Implementation) ---
    else {
        // Hilfsfunktionen für die neuen Chars, um sie an den Original-Look anzupassen
        const drawBody = (skinColor, shirtGrad, pantsColor, shoeColor, widthMod = 1) => {
            ...
            // Trapezform für den Körper (Vektor-Stil wie Rambo)
            ctx.beginPath(); 
            ctx.moveTo(-12 * widthMod, -18); 
            ctx.lineTo(12 * widthMod, -18); 
            ctx.lineTo(10 * widthMod, 12); 
            ctx.lineTo(-10 * widthMod, 12); 
            ctx.fill();
            // Kopf
            ...
            if (d !== 'side') {
                ...
                // Hände
                ctx.fillStyle = skinColor; 
                ctx.beginPath(); ctx.arc(-16 * widthMod, 6, 4, 0, Math.PI*2); ctx.fill();
                ctx.beginPath(); ctx.arc(16 * widthMod, 6, 4, 0, Math.PI*2); ctx.fill();
            }
        };
        ...
        const drawFace = (glasses=false, beard=false) => {
            ...
            if (!glasses) {
                ...
                ctx.fillStyle = '#111'; rect(-9, -25, 18, 6, '#111'); // Brille
            }
            if (beard) {
        ...
        if (id === 'cristiano') {
            const jersey = gradient(-20, 12, '#ff3333', '#cc0000');
            ...
            const suit = gradient(-20, 12, '#333', '#000');
            ...
            ctx.fillStyle = skin; rect(-10, -4, 20, 6, skin); // Bauchfrei
            ctx.fillStyle = '#000'; ctx.beginPath(); ctx.arc(0, -24, 11, Math.PI, 0); ctx.fill(); 
            if(d!=='back') { rect(-12, -24, 6, 24, '#000'); rect(6, -24, 6, 24, '#000'); }
            else rect(-11, -24, 22, 24, '#000');
            drawFace();
        }
        else if (id === 'lebron') {
            const jersey = gradient(-20, 12, '#fdb927', '#552583');
